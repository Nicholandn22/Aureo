...
export function useTransactionHistory(walletAddress: string | undefined) {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    try {
      ...
      // Use a 2-block safety margin to avoid "block not found" errors from RPC lag on testnet
      const safeBlock = Math.max(0, currentBlock - 2);
      const fromBlock = Math.max(0, safeBlock - 5000); // Last ~5000 blocks (reduced for performance)

      // Fetch BuyGold events
      try {
        const buyFilter = aureoContract.filters.BuyGold(walletAddress);
        const buyEvents = await aureoContract.queryFilter(
          buyFilter,
          fromBlock,
          safeBlock,
        );
        ...
        for (const event of buyEvents) {
          ...
          if (log.args) {
            const usdcSpent = Number(
              ethers.formatUnits(log.args.usdcSpent || 0, 18),
            );
            const goldReceived = Number(
              ethers.formatUnits(log.args.goldReceived || 0, 18),
            );
            const priceUsed = Number(
              ethers.formatUnits(log.args.priceUsed || 0, 18),
            );
      ...
      try {
        ...
        const sellEvents = await aureoContract.queryFilter(
          sellFilter,
          fromBlock,
          safeBlock,
        );
        ...
        for (const event of sellEvents) {
          ...
          if (log.args) {
            const goldSold = Number(
              ethers.formatUnits(log.args.goldSold || 0, 18),
            );
            const usdcReceived = Number(
              ethers.formatUnits(log.args.usdcReceived || 0, 18),
            );
            const priceUsed = Number(
              ethers.formatUnits(log.args.priceUsed || 0, 18),
            );
      ...
      try {
        const transferInFilter = goldContract.filters.Transfer(
          null,
          walletAddress,
        );
        const transferInEvents = await goldContract.queryFilter(
          transferInFilter,
          fromBlock,
          currentBlock,
        );
        ...
        for (const event of transferInEvents) {
          ...
          if (
            log.args &&
            log.args.from !== CONTRACT_ADDRESSES.AUREO_POOL &&
      ...
      try {
        const transferOutFilter = goldContract.filters.Transfer(
          walletAddress,
          null,
        );
        const transferOutEvents = await goldContract.queryFilter(
          transferOutFilter,
          fromBlock,
          currentBlock,
        );
        ...
        for (const event of transferOutEvents) {
          ...
          if (
            log.args &&
            log.args.to !== CONTRACT_ADDRESSES.AUREO_POOL &&
            log.args.to !== ethers.ZeroAddress
          ) {
            const value = Number(ethers.formatUnits(log.args.value || 0, 18));
      ...
      try {
        const transferInFilter = usdcContract.filters.Transfer(
          null,
          walletAddress,
        );
        const transferInEvents = await usdcContract.queryFilter(
          transferInFilter,
          fromBlock,
          currentBlock,
        );
        ...
        for (const event of transferInEvents) {
          ...
          if (
            ...
            const value = Number(
              ethers.formatUnits(log.args.value || 0, decimals),
            );
            const from = log.args.from as string;
      ...
      try {
        const transferOutFilter = usdcContract.filters.Transfer(
          walletAddress,
          null,
        );
        const transferOutEvents = await usdcContract.queryFilter(
          transferOutFilter,
          fromBlock,
          currentBlock,
        );
